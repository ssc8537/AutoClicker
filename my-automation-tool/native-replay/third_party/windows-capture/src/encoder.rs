...
impl VideoSettingsBuilder {
    ...
    /// Sets the uncompressed source size separately from the encoded output size.
    /// Media Foundation then scales the complete source frame instead of cropping it.
    pub const fn source_size(mut self, width: u32, height: u32) -> Self {
...
impl ContainerSettingsBuilder {
    ...
    /// Selects whether Media Foundation may use a hardware encoder.
...
impl VideoEncoder {
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        let source_width = video_settings.source_width;
        let source_height = video_settings.source_height;
        ...
        let hardware_acceleration = container_settings.hardware_acceleration;
        ...
        let media_stream_source = if is_video_disabled {
            MediaStreamSource::CreateFromDescriptor(&audio_stream_descriptor)?
        } else if is_audio_disabled {
