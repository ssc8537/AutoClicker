...
- [x] 1.3 创建 main.py
- [x] 1.4 F9 DOWN 热键循环触发可取消的 type_string("Hello World")
- [x] 1.5 用户手动实测：F12 启用后按住 F9 运行、松开/F12 停止（通过）
...
- [x] 2.6 设置开关
- [x] 2.7 核心手动测试验证（开始/停止 OSD + F9 DOWN 模式，通过）
- [x] 2.8 修复审查记录乱码（审查 #7 完整重构还原）
- [x] 2.9 文档编码加固（SETUP_GUIDE.md 添加 UTF-8 BOM）
- [x] 2.10 修复热键钩子跨线程访问与 F9 连按重叠漏洞
- [x] 2.11 修复 F9 长按重复触发，并按确认的触发语义提供停止反馈（自动验证通过）
- [ ] 2.12 补测 OSD 自动淡出与 popup_enabled 配置开关（不阻塞阶段 2）

## 阶段2：最小 JSON 序列播放器（需求已锁定，尚未实现）

规格唯一来源：`docs/阶段2-最小需求规格.md`。

- [x] 3.1 完成真实需求对齐审查并锁定阶段 2 最小边界
- [x] 3.2 创建并自动测试可中断的 `src/core/sequence_player.py`
- [x] 3.3 实现一个 JSON 小写 `hello world` 物理键盘宏的加载与严格校验
- [x] 3.4 将这个固定 F9/F12 宏接入现有热键管理器（模式、次数、速度）
- [x] 3.5 保存 JSON 后在下一次 F9 自动重载配置（无须重启）

> 历史验证阶段：JSON 运行格式现已被 Python 宏替代，不再进入程序运行路径。

## 阶段3：单 Python 宏脚本引擎

规格唯一来源：`docs/阶段3-Python宏规格.md`。

- [x] 4.1 创建可信 Python 宏加载、严格元数据校验和源码热重载
- [x] 4.2 创建提供 `tap` / `sleep` 的可中断 `ScriptPlayer`
- [x] 4.3 让单 Python 宏接入固定 F9/F12、模式、次数和速度
- [ ] 4.4 用户手动验收 Python `hello_world.py`
