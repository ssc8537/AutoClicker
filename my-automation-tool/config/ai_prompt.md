...
### `player.mouse_move(x, y, duration_ms=0)`
发送鼠标相对当前位置的 X/Y 位移，不读取当前位置，也不是绝对屏幕坐标。`x`、`y` 必须是 -10000 到 10000 的整数：X 正数向右、X 负数向左，Y 正数向下、Y 负数向上。`duration_ms=0` 一次发送完整位移；大于 0 时在指定真实时长内分步平滑移动，范围为 1–10000ms，不受 SPEED 缩放。移动期间收到停止、松开 down 触发键、第二次 switch 按键或退出会立即停止后续步。它不能录制或回放真实鼠标轨迹。

### `player.is_physical_pressed(key)`
只读判断一个真实物理键当前是否仍按住，`key` 使用第九节内部值，例如 `mouse_left`。Windows 全局钩子已排除本程序带 `MAPL` 标记的模拟输入，因此宏自己发送的键不会让这里变成按下。适合“反斜杠开启监听，物理左键按住时压枪、松开时等待”的联动逻辑；等待时必须搭配短暂 `player.sleep()`，禁止无等待空转。
...
`def run(player)` 是程序规定的唯一入口，不是某个具体技能。程序启动宏时只会调用它；因此每个宏都必须保留这个函数。为了让代码像“动作、平A、重击、闪避、压枪”一样容易阅读，可以在 `run` 上方定义自己的辅助函数，再由 `run(player)` 按顺序调用。

辅助函数只是代码整理方式，不会增加程序没有的能力：

| 辅助函数示例 | 实际作用 | 常用底层 API |
|---|---|---|
| `_动作(player, 动作名称, 按住毫秒, 等待毫秒=0)` | 调用项目设置里的共享动作 | `player.按键()`、`player.sleep()` |
| `_平A(player, 按住毫秒, 等待毫秒=0)` | 点击一次鼠标左键 | `player.mouse_click("left")` |
| `_重击(player, 按住毫秒, 等待毫秒=0)` | 用较长的左键按住时长表示重击 | `player.mouse_click("left")` |
| `_闪避(player, 按住毫秒, 等待毫秒=0)` | 点击一次鼠标右键 | `player.mouse_click("right")` |
| `_压枪(player, 每次下移单位=1, 每步毫秒=15, 单轮最长毫秒=5_000)` | 左键持续按住，同时按相对位移向下移动 | `mouse_down()`、`mouse_move()`、`mouse_up()` |

推荐的初学者结构如下。函数名称可以改成用户习惯的中文，但底层 API、参数单位和 `finally` 释放逻辑不能省略：
...
压枪或其他需要持续按住鼠标的动作必须使用 `try/finally`，保证停止或报错时释放左键：
...
def _压枪(player, 每次下移单位=1, 每步毫秒=15, 单轮最长毫秒=5_000):
    ...
    try:
        # 单轮最多运行 5000ms。5000 // 15 = 333，实际约 4995ms。
        # COUNT=0 会在按住触发键时开始下一轮；松开后仍会立即停止。
...
压枪示例将单轮安全上限写成 `5_000ms`，方便用户直接理解为 5 秒。`COUNT = 0` 会在触发键仍按住时重复 `run(player)`；松开触发键会立即停止当前移动并释放左键。固定重复的连招片段可以使用 `for _ in range(明确次数)`，但不要用 `while True` 自己制造无法停止的循环。
...
def run(player):
    # X 正数向右、Y 负数向上；在100ms内分步完成，不读取光标坐标。
    player.mouse_move(120, -30, duration_ms=100)
    player.sleep(200)
    # 回到相对方向的另一侧；只做受控位移，不记录鼠标轨迹。
...
Python宏不允许控制录像或直接读取事件文件；录像和日志由“开发连招”页面独立生成。本程序的Python宏不支持：键盘长按 API、两个键组合触发一个宏、绝对鼠标坐标、读取当前位置、鼠标轨迹录制/回放、滚轮自动化、图像识别、OCR、角色或冷却自动检测、窗口绑定、手柄、案例 JSON/QIM、网络请求、子进程或外部输入库。相对鼠标移动只能使用第三节的 `player.mouse_move()`，不要输出其他坐标或外部输入 API 的伪代码。
...
5. 没有 time.sleep、while True、线程、子进程、网络、第三方输入、绝对坐标/当前位置读取、轨迹或识别；若移动鼠标，只能调用 `player.mouse_move()`。
