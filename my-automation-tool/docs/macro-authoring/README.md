...
当前发布 `player.tap(key, hold_ms=20)`、`player.sleep(ms)` 及 `player.切换(1|2|3)`、`player.战技()`、`player.声骸()`、`player.大招()`、`player.跳跃()`、`player.处决()`。语义函数使用“设置”页保存的共享键盘物理键；鼠标、新热键、游戏状态识别和自动判断仍未发布，不能写入宏。
