...
本项目的宏是可信本地 Python 文件。编辑、列表扫描和 AI 提示词不会执行宏代码。热键、模式、次数、速度和启用状态请在“触发”页直接修改，改动立即保存；同键的启用宏允许并发。F12 会全局启停所有宏。
...
|---|---|
| `NAME` | 非空显示名称。 |
| `HOTKEY` | 由触发页管理；可录入一个键盘键或鼠标后退/前进侧键，F12 永远保留。 |
...
复制上面的完整文件结构，修改 `NAME` 和唯一的 `HOTKEY`，在 `run(player)` 中按顺序写已发布的键盘或鼠标函数和 `sleep`。每个动作后加入足够等待；先保存，再在安全的目标窗口中按现有教程测试。不要使用 F2/F12、组合键、普通字母/数字键或鼠标键作为 `HOTKEY`；不要写坐标鼠标、图像识别、角色判断或未发布函数。
