...
- 实时按键窗口支持高亮先后色、3/5/10条事件、透明度、四角缩放、位置记忆和`HH:mm:ss:fff`本地时间。
...
- 保留用户删除的`macros/12eqr左键_今汐.py`、新增的`macros/12eqr左键-今汐.py`和修改的`macros/13EQFR左键.py`，发布时不还原。
- 中文输入法下发送的物理字母仍会进入目标窗口IME组合态；用户已接受英文完全正确、中文轻微偶发现象。
- 不支持或禁止：鼠标轨迹、滚轮自动化、OCR/图像识别、游戏内存、DLL/驱动注入、OBS运行时依赖、手柄和反作弊绕过。
- 本轮没有上传`build/`、`dist/`、`captures/`、日志、优秀案例源码或本机运行配置。
