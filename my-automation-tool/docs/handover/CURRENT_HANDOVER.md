...
用户已明确反馈本轮“完美通过”并要求覆盖 GitHub 主分支。Stage 16 Windows 验收已归档；旧 `CURRENT_ACCEPTANCE.md` 已移除。`VV026-startup-enabled` 已发布，包含启动默认启用修复、稳定测试调整、文档更新和用户当前宏设置；禁止强推。

发布事实：最终版本提交 `685d85f6cb4f9f713fb6dcbf7af63b7bbc537ca1`，注释标签 `VV026-startup-enabled` 解引用到同一提交；GitHub 默认主干 `master` 与 `HEAD` 已实时核验到该提交。当前无既定未完成阶段。
