...
## 二、热键绑定与两种阶段 2 触发模式（深度参考 Quickinput trigger.cpp）

### 2.1 触发模式枚举

Quickinput 的 `macro.h` 定义了枚举 `enum TriggerMode { sw, down, up }`。阶段 2 只采用其中的 `sw` 和 `down`，不实现 `up`。
...
class TriggerMode(IntEnum):
    ...
    DOWN = 1     # 按下执行：无限循环松开停止；有限次数完成后停止
```

### 2.2 状态机逻辑（从 trigger.cpp switch/case 提取）

Quickinput 的 `trigger.cpp` 通过 `switch(macro.mode)` 分发三种源模式；阶段 2 只采用其中两种。
...
class HotkeyManager:
    ...
    def _on_key_event(self, key: str, is_pressed: bool):
        ...
        # 阶段 2 的两种触发模式（从 Quickinput trigger.cpp 提取）
...
| Quickinput trigger.cpp | switch(macro.mode) | 阶段 2 的 switch/down 触发逻辑 |
