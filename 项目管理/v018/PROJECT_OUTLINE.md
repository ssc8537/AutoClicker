...
### 当前版本
v018（2026-07-16，阶段 3 单 Python 宏脚本引擎，待用户验收）
...
### 下一个 AI 要做什么
阶段 2 的 JSON 历史验收已通过。当前阶段改为单 Python 宏：按 F9 前直接热重载 `scripts/hello_world.py`；保存 `COUNT = 1` 后轻按 F9 完整输出一次，保存 `COUNT = 0` 后按住循环、松开停止，均无需重启。
