...
### 当前版本
v012（2026-07-15，快照已创建）

### 上一个 AI 做了什么（审查 #12，2026-07-15）
- 更新优秀案例优先和防止问题复发规则，经验教训已同步记录
- 将 REVIEW_LOG.md 的 #5-#9 归档到 `my-automation-tool/docs/reviews/`
- 修复热键钩子线程直接操作 Qt UI 的跨线程风险，并加入 F9 非阻塞连按保护
- 为 SETUP_GUIDE.md 加入 UTF-8 BOM，创建 v012 快照

### 下一个 AI 要做什么
详见 REVIEW_LOG.md 末尾的「给下一个 AI 的提示词」。
优先级：
1. 等待并记录用户的阶段 1.5 OSD 手动测试结果
2. 开始阶段 2：脚本管理系统（先参考优秀案例）
...
| 屏幕提示文本功能 | 90%（代码完成+线程漏洞修复，待用户测试） | OSD 浮层、配置和主线程信号分发均已完成；2.7 仍待用户手动验证 |
