...
### 下一个 AI 要做什么
阶段 2 的需求对齐已完成，唯一规格为 `my-automation-tool/docs/阶段2-最小需求规格.md`。下一步按该文件第六节的三个子任务实现，先做可中断 `SequencePlayer`，不得扩大范围。
...
| 阶段2：最小 JSON 序列播放器 | 需求 100%，实现 0% | 一个测试宏、可中断播放器、热键/模式/次数/速度 |
