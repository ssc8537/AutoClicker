...
class _HotkeyDispatcher(QObject):
    ...
    def __init__(
        self, status_label: QLabel, osd_popup: OsdPopup, macro_runtime: MacroRuntime
    ):
        super().__init__()
    ...
    def on_f9_hook(self) -> None:
        ...
        try:
            ...
            logger.warning("F9 未启动：JSON 宏无效：%s", exc)
            if self._on_natural_finish is not None:
                self._on_natural_finish()
            self.macro_error_signal.emit("宏配置无效，未启动")
            return
        self.f9_signal.emit()
    ...
    def _start_f9(self) -> None:
        ...
        if not self._player.play(macro.steps, macro.count, macro.speed):
            logger.info("F9 启动被忽略：测试宏已经运行")
            return
        self._execution_active = True
        logger.info("F9 启动 JSON 测试宏：%s，count=%s", macro.name, macro.count)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, macro_runtime: MacroRuntime):
        super().__init__()
    ...
    def _setup_hotkey(self) -> None:
        ...
        logger.info("阶段 2 JSON 宏已加载：F9=%s，F12=全局启停", macro.mode)
...
def main() -> int:
    ...
    try:
        macro_runtime = MacroRuntime(_MACRO_PATH)
