...
### 当前版本
v021（2026-07-16，四页 UI 外壳自动测试完成，等待用户人工验收）
...
### 下一个 AI 要做什么
先等待并分析用户按 `docs/test-plans/V021_UI_SHELL_MANUAL_TEST.md` 给出的验收结果。通过前不扩展 UI 功能；失败时只修复记录到的 v021 外壳问题并重新自动测试。通过后再由 RequirementCertifier 审查下一阶段的 Python 宏库扫描或配置持久化候选任务。

### 用户需要做什么
执行 `my-automation-tool/docs/test-plans/V021_UI_SHELL_MANUAL_TEST.md`：先检查四页外观，再在记事本回归 F9/F12 并确认 F2 无反应。
...
| 阶段3：单 Python 宏脚本引擎 | 实现、自动测试与用户验收 100% | 可信 Python 宏、tap/sleep、固定 F9/F12、热重载 |
| v021：四页 UI 外壳 | 自动测试 100%，待用户验收 | Candy 粉红四页、F9/F12/OSD 不回归、F2 占位 |
