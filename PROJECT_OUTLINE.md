...
### 当前版本
v021.1（2026-07-16，`bc06485` 已安全快进发布至 GitHub `master`；运行与安全人工验收通过，V022 等待用户视觉规格确认）
...
### 下一个 AI 要做什么
等待用户按 `my-automation-tool/docs/test-plans/V022_VISUAL_SPEC_CONFIRMATION.md` 确认 V022 视觉规格；确认后由 RequirementCertifier 决定是否允许单独规划 UI 改进。不得直接实现宏库管理、编辑器或候选扩展。

### 用户需要做什么
本轮用户无需重复运行 F9/F12/F2；请阅读 `my-automation-tool/docs/test-plans/V022_VISUAL_SPEC_CONFIRMATION.md`，确认视觉规格的取舍。真正 UI 改动后才重新执行 Windows 外观和记事本安全验收。
...
| V022：视觉与窗口规则对齐审查 | 规格已完成，等待用户确认，禁止编码 | 以优秀案例 1 和四张参考图锁定美化规格 |
| C1：Python 宏库管理 | 候选核心，未 READY | 扫描、选择、错误展示、编辑与保存范围待锁定 |
...
本轮治理、路线图、V022 规格和角色配置在本记录形成时仍是未提交草稿；已完成终检、路径和结构复核，下一步必须先审查暂存范围、创建仅文档/角色配置提交，再按 GitHub SOP 推送功能分支与 `master` 并核验远端 SHA。该记录不表示草稿已经发布；下一位 AI 必须重新读取 Git 状态。
...
- 默认分支 `master`：`bc06485`（v021.1 已发布）；功能分支 `codex/v021-ui-shell` 同步指向该提交
