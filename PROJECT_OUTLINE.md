...
### 当前版本
v021.1 / V022（2026-07-16，运行与安全人工验收通过；用户已确认 V022 视觉规格，V023 实施 READY，交给下一位 AI）
...
### 下一个 AI 要做什么
下一位 AI 可按 V023 READY 实施“仅四页视觉对齐”：只能调整 `MainWindow._setup_ui()` 与四页布局/样式，并须完成离屏回归和 Windows 人工验收。不得直接实现宏库管理、编辑器或候选扩展。
...
| V023：四页 UI 视觉对齐实现 | 0%，实施 READY，交给下一位 AI | 仅布局、文案和 Candy 样式；运行边界不变 |
